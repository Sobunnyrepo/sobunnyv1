                ...
                <div class="tableStyle_three mt-4">
                    <div class="table_wrapper custom_Table">
                        <div class="search_category_result">
                            @include('backend.pages.category.search-category')
                        </div>
                    </div>
                </div>
            </div>
...
@section('scripts')
    @can('category-bulk-delete')
        <x-bulk-action.bulk-action-js :url="route('admin.category.bulk.action')"/>
    @endcan
        ...
        (function(){
            ...
            $(document).ready(function(){
                ...
                function notices(page,string_search){
                    $.ajax({
                        url:"{{ route('admin.category.paginate').'?page='}}" + page,
                        data:{string_search:string_search},
                        success:function(res){
                            $('.search_category_result').html(res);
                        }
