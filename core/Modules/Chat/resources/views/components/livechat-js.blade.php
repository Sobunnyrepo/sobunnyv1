...
<script>
    class LiveChat {
        ...
        removeChannel(user_id, member_id, type){
            if(type === 'user')
                this.channel = this.pusher.unsubscribe(`private-livechat-member-channel.${user_id}.${member_id}`);
            else
                this.channel = this.pusher.unsubscribe(`private-livechat-user-channel.${member_id}.${user_id}`);
        }
